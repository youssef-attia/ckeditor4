				...
				commit: function( data ) {
					...
					if ( CKEDITOR.env.ie ) {
						...
						if ( elementType != myType ) {
							var replace = CKEDITOR.dom.element.createFromHtml(CKEDITOR.tools.htmlSafeByReview( '<input type="' + myType + '"></input>', 'myType is set using the internal value and wrapped in a safe template'), editor.document );
